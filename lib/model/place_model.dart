...
Future<void> savePlacesToFirebase() async {
  final CollectionReference ref =
      FirebaseFirestore.instance.collection("myAppCpollection"); // Ensure correct name

  List<Future<void>> saveOperations = [];

  for (final Place place in listOfPlace) {
    final String id = ref.doc().id; // Generate unique ID
    saveOperations.add(ref.doc(id).set(place.toMap())); // Add to list of operations
  }

  await Future.wait(saveOperations); // Execute all in parallel
}
